    ...
    const fetchUserId = async () => {
        try {
            ...
            if (response.status === 200 && response.data.message === 'Authenticated') {
                // Now you need to fetch the user ID separately if it's not included in the auth response
                // You might need a separate API endpoint for fetching user details including userId
                const userResponse = await axios.get('/Api/getUserId', { withCredentials: true });
